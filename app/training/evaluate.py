...
from app.training.dataset import GelSegmentationDataset, TileDataset
...
def _mean_dice(model: BandUNet, dataset, device: torch.device) -> float:
    total, n = 0.0, 0
    with torch.no_grad():
        for imgs, masks in DataLoader(dataset, batch_size=1, shuffle=False):
...
def evaluate(checkpoint_path: str, device: torch.device) -> dict[str, float]:
    ...
    results = {}
    ds_cls = TileDataset if checkpoint.get("input_mode") == "tile" else GelSegmentationDataset
    val_ds = ds_cls(split="val", synth_per_epoch=300, seed=1234)
    results["synthetic_val"] = _mean_dice(model, Subset(val_ds, range(val_ds.synth_per_epoch)), device)
    for split in ("val", "test"):
        ds = ds_cls(split=split, synth_per_epoch=0)
